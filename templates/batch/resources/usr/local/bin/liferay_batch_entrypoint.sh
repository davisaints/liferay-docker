...
function main {
	...
	if [[ "${http_code}" -ge 400 ]]
	then
		echo "POST ${lxc_dxp_server_protocol}://${lxc_dxp_main_domain}${oauth2_token_uri} errored with HTTP code ${http_code}."
	...
	then
		...
		if [[ "${http_code}" -ge 400 ]]
		then
			echo "PUT ${lxc_dxp_server_protocol}://${lxc_dxp_main_domain}${href}${external_reference_code} errored with HTTP code ${http_code}."
	...
	do
		...
		if [[ "${http_code}" -ge 400 ]]
		then
			echo "POST ${lxc_dxp_server_protocol}://${lxc_dxp_main_domain}${href}${parameters} errored with HTTP code ${http_code}."
		...
		do
			...
			if [[ "${http_code}" -ge 400 ]]
			then
				echo "GET ${lxc_dxp_server_protocol}://${lxc_dxp_main_domain}/o/headless-batch-engine/v1.0/import-task/by-external-reference-code/${external_reference_code} errored with HTTP code ${http_code}."
