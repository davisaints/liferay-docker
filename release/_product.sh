...
function decrement_module_versions {
	...
	do
		...
		sed --expression --in-place "s/Bundle-Version: ${bundle_version}/Bundle-Version: ${major_minor_version}.${micro_version}/" "${bnd_bnd_file}"
	done
}
...
function update_release_info_date {
	...
	sed --expression --in-place "s/release.info.date=.*/release.info.date=$(date +"%B %d, %Y")/" release.properties
}
