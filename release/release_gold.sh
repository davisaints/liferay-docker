...
function update_salesforce_product_version {
	...
	if [ "${http_code}" != "200" ]
	then
		lc_log ERROR "Unable to update the Salesforce product version. HTTP code was ${http_code}. Create a general request ticket in the SFDC Jira project and ask for the inclusion of version $(get_product_group_version) in Salesforce."
